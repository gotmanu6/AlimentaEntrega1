...
## PoC Alimenta — AEP 2026.2

A interface da PoC fica em `http://localhost:8080/alimenta.html`.
Use `./mvnw clean verify` (PowerShell: `.\mvnw.cmd clean verify`) para executar
todos os testes e exigir cobertura Java mínima de 70%. O teste `AlimentaApiIT`
valida pontos, doações e subdocumentos em MongoDB real e isolado.
As instruções abaixo do projeto-base continuam aplicáveis à infraestrutura.
...
`JAVA_HOME` deve apontar para a pasta do JDK 21, e seu diretório `bin` deve estar no `PATH`.
