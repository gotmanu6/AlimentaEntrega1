...
class LinguagemControllerTest {
    ...
    @ParameterizedTest
    @ValueSource(strings = {"{", "", "{\"nome\":\"Java\",\"dataCriacao\":\"data-invalida\",\"autor\":\"James Gosling\"}"})
    void deveRejeitarCorpoIlegivelNaCriacaoEAtualizacao(String json) throws Exception {
        for (var requisicao : List.of(post(BASE_PATH), put(BASE_PATH + "/java"))) {
            mockMvc.perform(requisicao.contentType(MediaType.APPLICATION_JSON).content(json))
                    .andExpect(status().isBadRequest())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.status").value(400))
