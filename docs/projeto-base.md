...
No Windows/PowerShell, use `.\mvnw.cmd` no lugar de `./mvnw` nos comandos abaixo.
Configure `JAVA_HOME` para a pasta do JDK 21 instalado (a pasta que contém `bin`)
e inclua `$env:JAVA_HOME\bin` no `PATH` do terminal.
...
Os testes verificam também os códigos `201` e `204` e o cabeçalho `Location` no contrato OpenAPI, as respostas `400` para corpos ilegíveis e a carga de desenvolvimento sem duplicação ou sobrescrita de registros existentes.
...
Além das cinco operações CRUD, a integração verifica o acesso pelo cabeçalho
`Location`, os erros `404` e a preservação do banco quando POST ou PUT recebem dados inválidos.
...
Na validação de 09/09/2026 com Java 21, os 37 testes passaram, incluindo 11 de integração: **84 de 84 linhas relevantes cobertas (100%)**. A classe de bootstrap e as classes de configuração são excluídas da métrica JaCoCo, conforme o `pom.xml`.
